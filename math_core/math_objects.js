    ...
    replace(letters, terms) {
        ...
        if (this.is_disjunction) {
            return new Disjunction([
                this.disjunction_args[0].replace(letters, terms),
                this.disjunction_args[1].replace(letters, terms)
            ]);
        }

        if (this.is_relation) {
            return new Relation(this.name, [
                this.specialsign_args[0].replace(letters, terms),
                this.specialsign_args[1].replace(letters, terms)
            ]);
        }

        if (this.is_substantive) {
            return new Substantive(this.name, [
                this.specialsign_args[0].replace(letters, terms),
                this.specialsign_args[1].replace(letters, terms)
            ]);
        }
...
class Disjunction extends Ratio {
    ...
    get disjunction_args() {
        ...
        const args = get_args(this);
        this._disjunction_args = [
            this.theory.create(args["ratio0_name"], args["ratio0_id"]),
            this.theory.create(args["ratio1_name"], args["ratio1_id"])
        ];
...
class Relation extends Ratio {
    ...
    get specialsign_args() {
        ...
        const args = get_args(this);
        this._specialsign_args = [
            this.theory.create(args["term0_name"], args["term0_id"]),
            this.theory.create(args["term1_name"], args["term1_id"])
        ];
